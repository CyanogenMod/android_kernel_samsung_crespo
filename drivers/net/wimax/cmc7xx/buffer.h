 ...
 * buffer.h
 *
 * Used q_send for the control and data packet sending,
 * uses the BUFFER_DESCRIPTOR *q_received_ctrl used for the
 * control packet receive, data packet sent directly
 */
...
{	\
	struct list_head *h;	\
	h = (struct list_head *) (_L).next;	\
	if (h) {	/* then fix up our our list to point to next elem */\
		(_L).next = h->next; /* rechain list pointer to next link */\
		if (!((_L).next))	/* rechain list pointer
							* to next link if the
